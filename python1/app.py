...
@app.route('/') #digunakan dalam kerangka kerja Flask untuk menentukan URL yang akan ditangani oleh fungsi tertentu.
def home(): 
...
def take_quiz():
    if 'username' in session and session['role'] == 'siswa':
        # Periksa apakah siswa telah menyelesaikan kuis
        if 'quiz_completed' in session:
            return redirect(url_for('view_score'))  # Redirect ke halaman view_score jika kuis sudah selesai
        elif quiz_data:
            if request.method == 'GET':
                question_index = session.get('last_question_index', 0)
                time_limit = int(quiz_data[question_index].get('time_limit'))
                session['quiz_start_time'] = time.time()  # Mulai hitung waktu saat pengguna mengakses halaman quiz
                ...
                user_answer = request.form.get('answer', '')  # Menggunakan get untuk mendapatkan nilai 'answer' dengan nilai default ''
                ...
                time_taken = time.time() - session['quiz_start_time']  # Hitung waktu yang telah berlalu sejak mulai mengerjakan kuis
                time_remaining = max(0, time_limit - time_taken)

                if question_index >= len(quiz_data) - 1:
                    # Jika pengguna telah menjawab semua pertanyaan
                    session['quiz_completed'] = True  # Tandai bahwa kuis telah selesai
                    save_quiz_data()  # Simpan data kuis setelah selesai
                    return redirect(url_for('view_score'))

                # Pindah ke soal berikutnya hanya jika jawaban ada atau waktu masih tersisa
                if user_answer or time_remaining > 0:
                    session['last_question_index'] = question_index + 1
                    session['quiz_start_time'] = time.time()  # Mulai hitung waktu lagi untuk soal berikutnya
                else:
                    # Jika siswa tidak menjawab dan waktu sudah habis, pindah ke soal berikutnya
                    session['last_question_index'] = question_index + 1

                question_index = session['last_question_index']
